...
class CommunityPostCreate(BaseModel):
    ...
    poll_question: str | None = Field(
        default=None, description="If set, attaches a Yes/No poll asking this question."
    )


class CommunityPollAdd(BaseModel):
    poll_question: str = Field(min_length=1)
...
class CommunityCommentCreate(BaseModel):
    ...
    parent_comment_id: uuid.UUID | None = Field(
        default=None, description="Set to reply to an existing comment on this post."
    )
...
class CommunityPostOut(BaseModel):
    ...
    poll_yes_count: int
    poll_no_count: int
    my_vote: bool | None  # the caller's poll vote (True=yes/False=no), or null
    comment_count: int
    created_at: datetime
